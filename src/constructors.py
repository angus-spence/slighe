...
class StopSequenceConstructor:
    def __int__(self, trip_id: dtypes.Trip, gtfs_loader: dload.BaseDataLoader) -> None: self.trip_id = trip_id; self.gtfs_loader = gtfs_loader
...
class CorridorConstructor:
    def __init__(self, corridor_id: int, route_ids: list[dtypes.Route]) -> None: self.corridor_id = corridor_id; self.route_ids = route_ids
    def __call__(self) -> dtypes.Corridor: return self.build() 
    def build(self) -> dtypes.Corridor: return NotImplementedError #TODO: Implement this methods
